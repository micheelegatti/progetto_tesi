        ...
        <label class="flex flex-col gap-1">
          <span class="text-xs text-gray-500">Allineamento</span>
            <div class="flex gap-1">
              <button
                v-for="dir in ['left', 'center', 'right', 'justify']"
                :key="dir"
                class="flex-1 py-1 text-xs rounded border transition-colors"
                :class="blockSelected.layout!.alignItems === dir
                  ? 'bg-blue-500 text-white border-blue-500'
                  : 'bg-white text-gray-500 border-gray-200 hover:border-blue-300'"
                @click="blockSelected.layout && (blockSelected.style!.textAlign = dir as Align)"
              >
              {{ dir === 'left' ? '≡' : dir === 'center' ? '≡' : dir === 'right' ? '≡' : '≡' }}
            </button>
          </div>
        </label>
